...
export default function Rapports() {
  ...
  return (
    <div className="px-4 py-5 space-y-5">
      <div>
        <h2 className="text-lg font-black text-gray-900">📈 Rapports & Statistiques</h2>
        <p className="text-xs text-gray-400">Vue d'ensemble du portefeuille MPME</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          ...
          <div key={s.label} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 text-center">
            ...
            <p className="font-black text-gray-900 text-xl mt-1">{s.value}</p>
      ...
      {sectorData.length > 0 && (
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
          <h3 className="font-bold text-sm text-gray-800 mb-3">🎯 Répartition par secteur</h3>
      ...
      {regions.length > 0 && (
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
          <h3 className="font-bold text-sm text-gray-800 mb-3">🗺️ Taux d'éligibilité par région</h3>
          <div className="space-y-2">
            {regions.map(({ region, total: t, eligible, pct }) => (
              <div key={region} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                <div>
                  <p className="text-xs font-semibold text-gray-800">{region}</p>
                  <p className="text-[10px] text-gray-400">{eligible}/{t} MPME éligibles</p>
                </div>
                <span className="text-sm font-black text-sedo-blue">{pct}%</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <button className="w-full py-3 bg-sedo-blue text-white rounded-xl text-sm font-bold flex items-center justify-center gap-2 active:scale-95 transition-transform">
