  ...
  return (
    <div className="px-4 py-5 lg:px-0 lg:py-0 space-y-5 lg:space-y-6">
      ...
      <div className="bg-gradient-to-br from-sedo-green to-sedo-green-dark rounded-2xl p-6 lg:p-8 text-white flex flex-col lg:flex-row lg:items-center lg:gap-12">
        <div className="flex flex-col items-center lg:items-start">
          ...
          <div className="relative w-36 h-36 lg:w-48 lg:h-48">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-4xl lg:text-6xl font-black">{total}</span>
              <span className="text-green-200 text-sm lg:text-base">/100</span>
            </div>
          </div>
        ...
        {/* Niveaux inline sur desktop */}
        <div className="hidden lg:block flex-1">
          <h3 className="font-bold text-base text-white/80 mb-3">Niveaux de maturité</h3>
          <div className="space-y-2">
            {[
              { range: '76 – 100', label: 'Éligible au crédit', active: total >= 76 },
              { range: '56 – 75', label: 'Presque éligible', active: total >= 56 && total < 76 },
              { range: '31 – 55', label: 'En progression', active: total >= 31 && total < 56 },
              { range: '0 – 30', label: 'Non éligible', active: total < 31 },
            ].map((n) => (
              <div key={n.range} className={`flex items-center justify-between p-3 rounded-xl ${n.active ? 'bg-white/20 font-bold' : 'bg-white/5 opacity-60'}`}>
                <span className="text-sm text-white">{n.range}</span>
                <span className="text-sm text-white">{n.label} {n.active ? '← vous' : ''}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="space-y-3 lg:space-y-4">
        <h3 className="font-bold text-gray-800 text-sm lg:text-base">Détail par critère</h3>
            ...
            return (
              <div key={c.key} className="bg-white rounded-2xl p-4 lg:p-6 shadow-sm border border-gray-100">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-xl lg:text-3xl font-black text-sedo-green">{Math.round(val)}</span>
                </div>
                <div className="w-full bg-gray-100 rounded-full h-2 lg:h-3">
                  <div className="bg-sedo-green h-full rounded-full transition-all duration-500" style={{ width: `${val}%` }} />
                </div>
                <p className="text-[10px] lg:text-xs text-gray-400 mt-2">{c.desc}</p>
              </div>
            );
          })}
        </div>
